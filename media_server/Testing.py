...
## Preprocess Testing
if preprocTest == True:
    input_dir_pre = "./input_frame_semantic"
    ...
    folder_names, images_folder_list = prepro.splitSegements_all(frame_risk_list)
    ...
    for filename, risk, level in frame_risk_list:
        if level != last_level or len(imageList_in_folder) >= prepro.max_images:
            ...
            folder_name = prepro.splitSegemnt(filename, level, folder_index, file_index, new_folder = True)
            folder_names.append(folder_name)
            imageList_in_folder = [filename]
        else :
            file_index = file_index+1
            _ = prepro.splitSegemnt(filename, level, folder_index, file_index, new_folder = False)
